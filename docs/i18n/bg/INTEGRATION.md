...
**Languages:** [English](../../INTEGRATION.md) · [Български](INTEGRATION.md) · [Всички езици](../../LANGUAGES.md)
...
Използвайте един thread-safe ViesClient на процес, не на request. close() прекъсва вътрешната работа, общите заявки получават CLIENT_CLOSED, а новите извиквания хвърлят IllegalStateException.
...
check/checkAsync връщат 4 variants. Async ползва virtual threads и cancellation-safe single-flight; не пазете милиони futures.
...
Error JSON съдържа errorCode, messageHu, messageEn, retryable. Spring: singleton Bean с destroyMethod=close и controller за всички variants. Plain JDK пример: examples/ViesDemoServer.java.

Requester е собственият ДДС номер. Cached `requestDate`/`consultationNumber` описват първоначалната консултация, а не нова проверка; стойността им зависи от местните правила. `ViesCache` трябва да е thread-safe, с кратък timeout, versioned namespace, metrics, bounded retry и пълна serialization. Read exception → `CACHE_ERROR` без fallback; write exception запазва `Valid`.

При scale: persist job/idempotency, малък local retry, exponential delayed retry, max attempts, DLQ. Всеки pod има локални лимити, затова трябват global limiter, Redis, durable queue, dedup.

Liveness не зависи от VIES. Измервайте p50/p95/p99, result/errorCode, cache, retry, overload, queue age/DLQ; маскирайте VAT/name/address.

## Пълен приложен адаптер
