...
Uuenda `pom.xml`, commiti versioon/changelog, loo soovitatavalt allkirjastatud annoteeritud tag `git tag -s v1.2.0 -m "v1.2.0"`, seejärel `git push origin main --follow-tags`. Workflow kordab verify’d ja lisab binary/sources/Javadoc JAR-id ning SHA-256. Ära avalda ebaõnnestunud või ülevaatamata commit’i.
