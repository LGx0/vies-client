...
Päivitä `pom.xml`, commit/changelog, tee mieluiten allekirjoitettu annotoitu tagi `git tag -s v1.0.0 -m "v1.0.0"`, ja `git push origin main --follow-tags`. Workflow ajaa verify uudelleen ja liittää binary/sources/Javadoc JARit sekä SHA-256:t. Älä julkaise epäonnistunutta tai katselmoimatonta committia.
