...
Nuashonraigh `pom.xml`, commit leagan/changelog, cruthaigh tag sínithe anótáilte más féidir `git tag -s v1.2.0 -m "v1.2.0"`, ansin `git push origin main --follow-tags`. Athrithfidh workflow verify agus ceanglóidh binary/sources/Javadoc JAR + SHA-256. Ná heisigh commit teipthe nó neamh-athbhreithnithe.
