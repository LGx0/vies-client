# Italiano (`it`) — Pubblicazione GitHub

[Selettore lingua](../../LANGUAGES.md) · [Originale inglese](../../GITHUB_SETUP.md)

> Prevale inglese; `LICENSE` resta non tradotto e vincolante.

Prima: confermare diritti, scansionare directory/storia Git, rimuovere dati cliente/requester, token, `.env` e file macchina, verificare Apache-2.0 `LICENSE`/`NOTICE`, eseguire `./mvnw --batch-mode --no-transfer-progress clean verify`.

Creare repo pubblico `vies-client`, branch `main`, senza duplicati generati. Dopo sostituzione OWNER e review staged diff: `git init -b main`, add/commit, `gh repo create OWNER/vies-client --public --source . --remote origin --push`.
